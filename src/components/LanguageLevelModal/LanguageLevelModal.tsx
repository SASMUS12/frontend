...
const LanguageLevelModal: React.FC<LanguageLevelModalProps> = ({isModalOpen, setModalOpen, pageName}) => {
    ...
    return (
        <Modal isOpen={isModalOpen} onClose={handleCloseModal} className={styles.modal}>
            {Levels.map((level) => (
                <article className={styles.modal__item}>
                    <div className={styles.modal__languages}>
                        ...
                        <p className={styles.modal__languages_text}>
                            {level.description}
                        </p>
                    </div>
                    <img src={level.level} alt={level.name}
                         className={pageName === "Sort" ? styles.modal__image : styles.modal__image_hidden}/>
                </article>
            ))}
        </Modal>
    );
