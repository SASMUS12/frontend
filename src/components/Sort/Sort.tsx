...
const Sort: React.FC<SortProps> = ({
  ...
  const [selectedSuggestionIndex, setSelectedSuggestionIndex] = useState<
    number | null
  >(null); // Cостояние для отслеживания выбранной подсказки
  ...
  const handleAddLanguage = (
    language: UserForeignLanguage | UserNativeLanguage,
  ) => {
  ...
  const handleRemoveLanguage = (
    ...
    setSelectedLanguages((prevLanguages) =>
      prevLanguages.filter(
        (lang) => (lang as Language).isocode !== language.isocode,
      ),
    );
  };
  ...
  const handleFindButtonClick = () => {
    ...
    if (leftValue <= rightValue) {
      ...
      const foreignLanguages = selectedLanguages
        ...
        .filter((language) => language !== null) as {
        language: string;
        skill_level: SkillLevelEnum;
      }[];
  ...
  return (
    <div className={isOpen ? styles.popup__sort : styles.popup__sort_hidden}>
      ...
      <div className={styles.languageHelp}>
        ...
        <Button className={styles.languageHelp__button} />
      </div>
      <LanguageLevel
